...
def compute_p_hat_EZ(Xs,Xprime,n_train):
...
def reliable(X_s,delta_s,alpha,L,S_hat, n_train):
    # XS : n_train is used in compute_p_hat_EZ
    # 75/94 * (c/64 L)^(d/alpha))
    # ---> cc * c^d_alpha
    d=myData.d # XS TODO : make this a parameter
    d_alpha=d/alpha
#    cc=75.0/94 * (1.0/(64*L)**d_alpha) # version originale (cf. article)
    cc=75.0/94 * (1.0/2*L)**d_alpha # version modifiée (sinon aucun point n'est True)
    # XS TODO : get cc out of here
    for X_prime,Y_prime, LB in S_hat: # LB = c = lower bound guarantee on X'
#        print (X_prime,Y_prime, LB)
        p_hat=compute_p_hat(X_s,X_prime)#,n_train)
        p_hat_prime=compute_p_hat(X_prime,X_s)#,n_train)
...
# def find_knn_approx(x_s,k=2):
#     kk=int(k)
#     lshf = LSHForest()
#     lshf.fit(myData.Xtrain) 
#     a=knn.kneighbors(x_s.reshape(1, -1), return_distance=False) 
#     return a[0,1:] # remove index 0 == x_s itself    
...
#    log_1_delta = -log(delta) # = log (1/delta)
    n_cut=500 # XS TODO : make it a function of budget ?
    ...
    if (n_neighbors_max > n_cut):
        print ('confidentLabel : warning : n_neighbors_max is very big : %d > %d'%(n_neighbors_max,n_cut))
        print ('setting it to n_cut=%d'%n_cut)
        n_neighbors_max = n_cut
    
    # XS : try approximate KNN 
#    KNN=find_knn_approx(X_s,n_neighbors_max) # compute here the (n_neighbors_max) nearest neighbors of X
...
if __name__ == '__main__':
    ...
    for repet in range(10):
        ...
        dataType='gaussian05'
        dataFile=''
#        dataFile='./datasets/data_banknote_authentication.csv' --- trop petit
#        dataFile='./datasets/Skin_NonSkin_01.txt'
        _xymin=-1
        _xymax=1
        ...
        if (dataType!='fromFile'): 
            ...
            myData=dataSynth.dataSynth(-1,-1,dataType) # dimension -1 is irrelevant
            myData.setFile(dataFile)
            myData.makeData()
            myData.splitFromFile()
            # XS TODO use get methods
            n_test=myData.n_test
            n_train=myData.n_train
            n_unlabeled=0
            n_coldStart=0
           
            # XS TODO : use accessors to n_train
            n=myData.n_train+myData.n_test
            w=myData.n_train
            budget=w
        t=budget
        nb_points_used=[]
...
#            plt.figure()
    ...
    #        print ('point courant : X_s = ', X_s)
            ...
            if not reliable(X_s,delta_s,alpha,L,S_hat,n_train):
                ...
                if (dataType=='fromFile'): 
                    print ("%d : reliable=FALSE : potentially informative point"%s)
                else:
    ...
    #            print('le vrai label est %d, confidentLabel retourne %d'%(myData.Ytrain[s-1],Y_s))
                ...
                if is_informative_point:
                    ...
                    n_hat+=1
                    # not out of if : it is just a counter
                ...
                nb_reliab +=1
                if viz:
                    plt.scatter(X_s[0],X_s[1],color='black', marker='.')#,s=5)
                    plt.draw()
        ...
        print('longueur de n_act : %d' %len(n_act))
        print('n_info = %d'%n_info)
        print('n_hat = %d'%n_hat)
        # print('------- S_hat ------')
        # print(S_hat)
        # print('------- S_hat [0] ------')

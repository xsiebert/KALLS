...
#        self.xs = [np.random.uniform(-1, 1, d) for _ in range (n)]
    ...
    def setFile(self,f):
        ...
        if not path.isfile(f):
            print ('<dataSynth::setFile>: file does not exist')
    def getSize(self):
        return self.n
 ...
 #        return 0 if np.random.random() >= p else 1 # watch out sign!! 
    ...
    def makeX(self):
        ...
        elif (self.dataType[0:8]=='fromFile'):
            self.makeX_fromFile() # XS hack : fabrique le Y aussi 
        else:
    ...
    def makeData(self):
        self.makeX()
        if (self.dataType[0:8]!='gaussian' and self.dataType[0:5]!='mnist' and self.dataType[0:8]!='fromFile'):
            self.makeY()
    ...
    def makeX_fromFile(self):
        if self.dataFile=='':
            print('<DataSynth::makeX_fromFile> : you must specify data file')
            exit()
        else:
            dataset = pd.read_csv(self.dataFile,header=None)
            self.d=len(dataset.columns)-1
            # self.X=dataset[0:self.d]
            # self.Y=dataset[self.d]
            input_features = dataset.columns[:self.d]    
            X = dataset[input_features].to_numpy()    
            # XS TODO : for the moment, consider data as uniform on [-1:1]
            max_abs_scaler = MinMaxScaler(feature_range=[_xymin,_xymax])
            self.X = max_abs_scaler.fit_transform(X)
            self.Y = dataset[self.d].to_numpy()
    ...
    def makeX_Gaussian(self):
        ...
        X, self.Y= make_blobs(n_samples=self.n, n_features=self.d, cluster_std=0.5, centers=ccc,
    ...
    def splitFromFile(self):
        # take 30% as test data
        self.n_coldStart=0
        self.n_unlabeled=0
